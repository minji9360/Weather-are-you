...
@app.route('/')
def home():
  nowWeather = getWeather()
  return render_template('index2.html', weather=nowWeather)

# 함수 파일로 따로 빼고 싶어서 weather2.py에 저장하고, import함
# 근데 여기서 어떻게 불러오는지 모르겠어서(weather2에서 사용한 변수를 가져올 수도 없고.. 함수로 사용하려고 해도 결과가 이상함)
# 임시로 여기 추가해둠
def getWeather():
  ...
  nowTime = now[1]
  print(now)
  nowLocation = { "x":126, "y":127 } # 임시 위치(창원)
